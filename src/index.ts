...
import {
  ...
  BUILTIN_STANDARD_APK_PATH_FROM_ENV,
  HOST,
  PORT,
  STRICT_REDIS,
  STRICT_TOOLCHAIN,
...
async function boot(): Promise<void> {
  ...
  if (BUILTIN_STANDARD_APK_PATH && !fs.existsSync(BUILTIN_STANDARD_APK_PATH)) {
    ...
    if (BUILTIN_STANDARD_APK_PATH_FROM_ENV && !config.activeStandardId && !config.previousStandardId) {
      throw new Error(
        `[startup] builtin standard APK not found at ${BUILTIN_STANDARD_APK_PATH} and no standard package configured.`
      );
    }
    console.warn(
      `[startup] builtin standard APK not found at ${BUILTIN_STANDARD_APK_PATH}; fallback standard package may be unavailable.`
    );
  }
  if (STRICT_TOOLCHAIN) {
